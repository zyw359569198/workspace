...
import com.reign.framework.hibernate.hql.*;
...
public class BaseDao<T extends IModel, PK extends Serializable> extends HibernateDaoSupport implements IBaseDao<T, PK>
{
    @Autowired
    @Qualifier("sessionFactory")
    public SessionFactory sessionFactory;
    @Autowired
    private HqlFactory factory;
	...
	public List<?> getResultByHQLAndParam(final String hql) {
        this.getHibernateTemplate().setCacheQueries(true);
        return this.getHibernateTemplate().find(this.factory.get(hql));
    }
    
    @Override
	public T getFirstResultByHQLAndParam(final String hql) {
        this.getHibernateTemplate().setCacheQueries(true);
        final List<T> resultList = this.getHibernateTemplate().find(this.factory.get(hql));
	...
	public List<?> getResultByHQLAndParam(final String hql, final Object... object) {
        this.getHibernateTemplate().setCacheQueries(true);
        return this.getHibernateTemplate().find(this.factory.get(hql), object);
    }
    
    @Override
	public T getFirstResultByHQLAndParam(final String hql, final Object... object) {
        this.getHibernateTemplate().setCacheQueries(true);
        final List<T> resultList = this.getHibernateTemplate().find(this.factory.get(hql), object);
        if (resultList.size() > 0) {
            return resultList.get(0);
        }
        return null;
    ...
    @Override
	public List<?> getResultByHQLAndParam(String hql, final PagingData page, final Object... object) {
        hql = this.factory.get(hql);
    ...
    @Override
	public List<?> getResultByHQLAndParamNoUpdate(String hql, final PagingData page, final Object... object) {
        hql = this.factory.get(hql);
        final Session session = this.getSession();
        final Query query = session.createQuery(hql);
        query.setCacheable(true);
    ...
    @Override
	public List<?> getResultByHQLAndParamForUpdate(String hql, final String alias) {
        hql = this.factory.get(hql);
        final Session session = this.getSession();
        final Query query = session.createQuery(hql);
    ...
    @Override
	public List<?> getResultByHQLAndParamForUpdate(String hql, final String alias, final Object... object) {
        hql = this.factory.get(hql);
        final Session session = this.getSession();
        final Query query = session.createQuery(hql);
        this.prepareQuery(query, object);
	...
	public Iterator<?> getIteratorByHQLAndParam(final String hql) {
        return this.getHibernateTemplate().iterate(this.factory.get(hql));
    }
    
    @Override
	public T getFirstIteratorByHQLAndParam(final String hql) {
        final Iterator<T> it = this.getHibernateTemplate().iterate(this.factory.get(hql));
        if (it.hasNext()) {
            return it.next();
        }
        return null;
	...
	public Iterator<?> getIteratorByHQLAndParam(final String hql, final Object... object) {
        return this.getHibernateTemplate().iterate(this.factory.get(hql), object);
    }
    
    @Override
	public T getFirstIteratorByHQLAndParam(final String hql, final Object... object) {
        final Iterator<T> it = this.getHibernateTemplate().iterate(this.factory.get(hql), object);
        if (it.hasNext()) {
            return it.next();
        }
        return null;
    ...
    @Override
	public Iterator<?> getIteratorByHQLAndParam(String hql, final PagingData page, final Object... object) {
        hql = this.factory.get(hql);
        final Session session = this.getSession();
        final StringBuffer counterHql = new StringBuffer();
        final int fromIndex = hql.toUpperCase().indexOf("FROM");
    ...
    @Override
	public Iterator<?> getIteratorByHQLAndParamNoUpdate(String hql, final PagingData page, final Object... object) {
        hql = this.factory.get(hql);
        final Session session = this.getSession();
        final Query query = session.createQuery(hql);
        query.setCacheable(true);
