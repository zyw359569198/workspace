...
public class HallsCache extends AbstractCache<Integer, Halls>
{
	...
	public void afterPropertiesSet() throws Exception {
        ...
        for (final Halls halls : list) {
            ...
            for (int length2 = (array = outputArrays).length, j = 0; j < length2; ++j) {
                ...
                final String[] single = outputArray.split(",");
                map.put(RewardType.getTypeInt(single[0]), Integer.valueOf(single[1]));
            }
    ...
    public Set<Integer> getPris(final int officerId) {
        final Set<Integer> set = new HashSet<Integer>();
        final List<Halls> optionalList = this.officialHallsMap.get(officerId);
        for (final Halls halls : optionalList) {
            set.add(halls.getPri());
        }
        return set;
    }
