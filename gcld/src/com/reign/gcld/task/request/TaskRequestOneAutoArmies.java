...
public class TaskRequestOneAutoArmies extends TaskRequestCount
{
    public TaskRequestOneAutoArmies(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter dataGetter, final int vId) {
        if (message instanceof TaskMessageOneAutoArmies) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageOneAutoArmies;
    }
}
