...
public class TaskRequestBlackMarketVisit extends TaskRequestCount
{
    public TaskRequestBlackMarketVisit(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageBlackMarketVisit) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageBlackMarketVisit;
    }
}
