...
public class TaskRequestStoreRefresh extends TaskRequestCount
{
    public TaskRequestStoreRefresh(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageStoreRefresh) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageStoreRefresh;
    }
}
