...
public class TaskRequestWorldTreasure extends TaskRequestCount
{
    ...
    public TaskRequestWorldTreasure(final String[] strings) {
        super(1);
        this.boxId = Integer.valueOf(strings[1]);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter dataGetter, final int vId) {
        if (message instanceof TaskMessageWorldTreasure) {
            final TaskMessageWorldTreasure taskMessageWorldTreasure = (TaskMessageWorldTreasure)message;
            final int requestId = taskMessageWorldTreasure.getBoxId();
            final PlayerTask playerTask = dataGetter.getPlayerTaskDao().read(vId);
            if (playerTask == null) {
                return;
            }
            if (this.boxId != 0 && requestId != this.getBoxId()) {
                return;
            }
    ...
    private boolean hasGottenBox(final PlayerDto playerDto, final IDataGetter taskDataGetter) {
        ...
        try {
            ...
            TaskRequestWorldTreasure.log.error("TaskRequestWorldTreasure hasGottenBox ", e);
            return false;
        }
    }
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageWorldTreasure;
    }
