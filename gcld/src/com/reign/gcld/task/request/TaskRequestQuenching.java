...
public class TaskRequestQuenching extends TaskRequestCount
{
    public TaskRequestQuenching(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter dataGetter, final int vId) {
        if (message instanceof TaskQuenchingMessage) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskQuenchingMessage;
    }
}
