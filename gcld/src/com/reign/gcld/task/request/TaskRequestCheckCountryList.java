...
public class TaskRequestCheckCountryList extends TaskRequestCount
{
    public TaskRequestCheckCountryList(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageCheckCountryList) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageCheckCountryList;
    }
}
