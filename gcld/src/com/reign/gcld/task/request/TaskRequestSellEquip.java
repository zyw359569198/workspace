...
public class TaskRequestSellEquip extends TaskRequestCount
{
    public TaskRequestSellEquip(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageSellEquip) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageSellEquip;
    }
}
