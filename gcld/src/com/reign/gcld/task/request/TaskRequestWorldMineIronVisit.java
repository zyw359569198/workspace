...
public class TaskRequestWorldMineIronVisit extends TaskRequestCount
{
    public TaskRequestWorldMineIronVisit(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageWorldMineIronVisit) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageWorldMineIronVisit;
    }
}
