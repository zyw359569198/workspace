...
public class TaskRequestChangeName extends TaskRequestCount
{
    public TaskRequestChangeName(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageChangeName) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageChangeName;
    }
}
