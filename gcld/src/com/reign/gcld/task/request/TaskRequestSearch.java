...
public class TaskRequestSearch extends TaskRequestCount
{
    public TaskRequestSearch(final String[] s) {
        super(s);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageSearch) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageSearch;
    }
}
