...
public class TaskRequestOneNationalRankBattle extends TaskRequestCount
{
    public TaskRequestOneNationalRankBattle(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter dataGetter, final int vId) {
        if (message instanceof TaskMessageOneNationalRankBattle) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageOneNationalRankBattle;
    }
}
