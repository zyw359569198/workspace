...
public class TaskRequestWorldMineJadeVisit extends TaskRequestCount
{
    public TaskRequestWorldMineJadeVisit(final String[] s) {
        super(1);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageWorldMineJadeVisit) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageWorldMineJadeVisit;
    }
}
