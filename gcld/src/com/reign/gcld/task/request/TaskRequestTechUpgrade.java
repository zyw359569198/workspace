...
public class TaskRequestTechUpgrade extends TaskRequestCount
{
    public TaskRequestTechUpgrade(final String[] s) {
        super(s);
    }
    
    @Override
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageTechUpgrade) {
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageTechUpgrade;
    }
}
