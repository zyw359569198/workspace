...
public class TaskRequestUseIncense extends TaskRequestCount
{
    private int id;
    
    public TaskRequestUseIncense(final String[] s) {
        super(Integer.parseInt(s[2]));
    ...
    public void handleMessage(final TaskMessage message, final IDataGetter taskDataGetter, final int vId) {
        if (message instanceof TaskMessageUseIncense) {
            if (((TaskMessageUseIncense)message).getId() != this.id) {
                return;
            }
    ...
    public boolean isConcernedMessage(final TaskMessage message) {
        return message instanceof TaskMessageUseIncense;
    }
}
