package com.reign.gcld.task.reward;

import com.reign.gcld.player.dto.*;
import java.util.*;
import com.reign.gcld.common.*;

public class TaskRewardFreeNiubiQuenchingTimes implements ITaskReward
{
    private int num;
    
    public TaskRewardFreeNiubiQuenchingTimes(final int num) {
        this.num = num;
    }
    
    public TaskRewardFreeNiubiQuenchingTimes(final String[] s) {
        if (s.length > 1) {
            this.num = Integer.parseInt(s[1]);
        }
        else {
            this.num = 0;
        }
    }
    
    @Override
    public Map<Integer, Reward> rewardPlayer(final PlayerDto playerDto, final IDataGetter taskDataGetter, final String prefixAttribute, final Object obj) {
        taskDataGetter.getPlayerQuenchingRelativeDao().addFreeNiubiTimes(playerDto.playerId, this.num, prefixAttribute);
        final Map<Integer, Reward> map = new HashMap<Integer, Reward>();
        map.put(101, new Reward(101, LocalMessages.T_COMM_10051, this.num));
        return map;
    }
    
    @Override
    public Map<Integer, Reward> getReward(final PlayerDto playerDto, final IDataGetter taskDataGetter, final Object obj) {
        final Map<Integer, Reward> map = new HashMap<Integer, Reward>();
        map.put(101, new Reward(101, LocalMessages.T_COMM_10051, this.num));
        return map;
    }
}
