...
public class TaskRewardPhantom implements ITaskReward
{
    private int num;
    
    public TaskRewardPhantom(final int num) {
        this.num = num;
    }
    
    public TaskRewardPhantom(final String[] s) {
        if (s.length > 1) {
    ...
    public Map<Integer, Reward> rewardPlayer(final PlayerDto playerDto, final IDataGetter taskDataGetter, final String prefixAttribute, final Object obj) {
        taskDataGetter.getPlayerBattleAttributeDao().addVip3PhantomCount(playerDto.playerId, this.num, prefixAttribute);
        final Map<Integer, Reward> map = new HashMap<Integer, Reward>();
        map.put(102, new Reward(102, LocalMessages.T_COMM_10050, this.num));
        return map;
    }
